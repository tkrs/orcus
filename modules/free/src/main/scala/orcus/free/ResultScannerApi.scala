...
trait ResultScannerApi[F[_]] {
  ...
  def nextOne(resultScanner: ResultScanner): ResultScannerF[Result]
...
object ResultScannerOp {
  final case class NextOne(resultScanner: ResultScanner)      extends ResultScannerOp[Result]
...
class ResultScannerOps[M[_]](implicit inj: Inject[ResultScannerOp, M]) extends ResultScannerApi[M] {
  ...
  override def nextOne(resultScanner: ResultScanner): ResultScannerF[Result] =
